...
Tests are stored in [tests](/tests) directory and [tests using requre](/tests-requre)
...
### Data regeneration
 * copy secrets direcotory to top directory of packit-service
 * remove all necessary files what you want to regenerate
   ```bash
   rm -r tests_requre/test_data/test_*
   ```
 * Run the regeneration (part of this step is also doing containers for ``worker`` and ``test_image``)
   ```bash
